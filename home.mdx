---
title: "Orderly Home Page"
mode: "custom"
---

export function openSearch() {
  document.getElementById("search-bar-entry").click();
}

<div
  className="relative w-full flex items-center justify-center"
  style={{ height: '24rem', backgroundColor: '#1F1F33', overflow: 'hidden' }}
>
  <div style={{ flex: 'none' }}>
    <img
      src="images/home_banner.jpg"
      style={{ height: '400px', width: '2000px', pointerEvents: 'none'  }}
    />
  </div>
  <div style={{ position: 'absolute', textAlign: 'center' }}>
    <div
      style={{
        color: 'white',
        fontWeight: '400',
        fontSize: '48px',
        margin: '0',
      }}
    >
      Orderly Documentation
    </div>
    <p
      style={{
        color: 'white',
        fontWeight: '400',
        fontSize: '20px',
        opacity: '0.7',
      }}
    >
      What can we help you build?
    </p>
    <button
      type="button"
      className="mx-auto w-full flex items-center text-sm leading-6 shadow-sm text-gray-400 bg-white ring-1 ring-gray-400/20 focus:outline-primary"
      id="home-search-entry"
      style={{
        maxWidth: '24rem',
        borderRadius: '4px',
        marginTop: '3rem',
        paddingLeft: '0.75rem',
        paddingRight: '0.75rem',
        paddingTop: '0.75rem',
        paddingBottom: '0.75rem',
      }}
      onClick={openSearch}
    >
      <svg
        className="h-4 w-4 ml-1.5 mr-3 flex-none bg-gray-500 hover:bg-gray-600 dark:bg-white/50 dark:hover:bg-white/70"
        style={{
          maskImage:
            'url("images/home_banner.jpg")',
          maskRepeat: 'no-repeat',
          maskPosition: 'center center',
        }}
      />
      Start a chat with us...
    </button>
  </div>
</div>
<div style={{marginTop: '6rem', marginBottom: '8rem', maxWidth: '70rem', marginLeft: 'auto',
    marginRight: 'auto', paddingLeft: '1.25rem',
    paddingRight: '1.25rem' }}>
<div
  style={{
    textAlign: 'center',
    fontSize: '24px',
    fontWeight: '600',
    color: '#121142',
    marginBottom: '3rem',
  }}
>
  <span className="text-primary">Choose a topic below</span>
</div>

<CardGroup cols={3}>
  <Card title="Guides" icon="book-open" href="/introduction">
    About Orderly.
  </Card>
  <Card title="Reference" icon="code-simple" href="/build-on-omnichain/building-on-omnichain">
    Comprehensive details about the Orderly API.
  </Card>
  <Card
    title="Examples"
    icon="grid-round"
    iconType="solid"
    href="https://github.com/OrderlyNetwork/example-dex"
  >
    Hands-on example of Orderly SDK.
  </Card>
  <Card title="Integrations" icon="link-simple" href="/introduction/trade-on-orderly/builders">
    A growing number of front-ends built on Orderly.
  </Card>
  <Card title="Tools" icon="wrench" href="/sdks">
    Tooling and SDKs to for integrating Orderly.
  </Card>
  <Card title="Releases" icon="party-horn" href="/changelog">
    News about features and changes in Orderly.
  </Card>
</CardGroup>

</div>
