---
title: "FAQ"
description: "Find answers to frequently asked questions about Orderly Network, covering supported chains, fees, and key concepts."
sidebarTitle: "FAQ"
---
...
Looking ahead, our grand vision is to create an omnichain protocol, connecting traders from both EVM and non-EVM chains within the same orderbook.
  ...
  ## For traders:
    Traders can start using any interface built on Orderly listed on [this](/introduction/trade-on-orderly/builders) page
...
<br />
Traders can explore an ecosystem of front-ends built on Orderly. A comprehensive list can be found [here](/introduction/trade-on-orderly/builders).
</Accordion>
...
<Accordion title="Can I list my token on Orderly?">
  Yes. Use [Perp Anything in Orderly One](https://dex.orderly.network/permissionless-listing) to
...
<Accordion title="What are the different order types?">
  **MARKET:** It matches until the full size is executed. If the size is too large (larger than the
  ...
  <Accordion title="How are orders authenticated before settlement?">
    Order requests are signed using users' Trading Keys. The Messaging Layer
    verifies the request signature before forwarding an order to the Engine
    Layer. Only matched trade data is included in the settlement batch;
    unmatched and canceled orders are not included.

    See [Orderly Orderbook Design](/introduction/trade-on-orderly/trading-basics/orderbook-design)
    for the visibility boundaries before and after matching.
  ...
  <Accordion title="A DEX I use has shut down. How do I exit my positions?">
    If a DEX you trade on has shut down, ceased operations, or is no longer available, use the **Emergency Exit** tool to close all positions and withdraw your funds from Orderly Network.

    Learn more: [Emergency Exit Offboarding Tool](/introduction/trade-on-orderly/emergency-exit-offboarding-tool)

  </Accordion>

  <Accordion title="Where can I find independent data on Orderly's trading volume?">
    [OpenChainBench](https://openchainbench.com/benchmarks/perp-volume-share) tracks Orderly's
    24h rolling notional volume alongside 14 other perp DEX venues, updated every 5 minutes using
    Orderly's public API. The benchmark ranks venues by volume and covers the full perp DEX cohort.
