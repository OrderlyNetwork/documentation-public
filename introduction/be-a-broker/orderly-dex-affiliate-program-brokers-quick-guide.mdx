...
where $B$ is the base commission, $M$ is the additional multilevel referral commission assigned to the $L_1$ affiliate's referral chain, $X_1 = B + M$, and $\text{BaseFee}_\text{Builder}$ depends on the builder fee tier. Custom $X_1$ cannot be lower than the base commission $B$ and cannot exceed 100% of $\text{Fee}_\text{Builder}$.
