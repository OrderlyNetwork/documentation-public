...
DEXs have the flexibility to determine the referral commission rate allocated to L1 affiliates. These L1 affiliates are the initial referrers who establish a multilevel network and do not report to any referrer above them.
...
where `(Base fee)Broker` depends on the broker fee tier of the DEX.
...
| **LN user**       | `XN`                                         | `XN × (Fee)Broker` (referee’s commission)    |
