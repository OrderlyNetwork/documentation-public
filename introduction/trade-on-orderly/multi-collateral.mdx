...
description: "Understand supported collateral types, deposit caps, discount rates, and cross-chain rules."
---

## Collaterals

Eligible collateral assets also support [Isolated Margin](/introduction/trade-on-orderly/perpetual-futures/isolated-margin#multi-collateral-support), including supported non-USDC collateral. Isolated margin remains denominated and accounted for in USDC. When projected USDC borrowing from opening an isolated-margin position exceeds the **50,000 USDC** limit, automatic conversion of collateral into USDC is triggered to reduce the borrowing amount.
...
| ^(4/5)              | Smooths impact of large position size                                      |
