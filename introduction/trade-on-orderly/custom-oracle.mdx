---
title: "Custom Oracle"
description: "Learn how Builders can provide custom oracle feeds for Perp Anything markets on Orderly."
---

Custom Oracle lets Builders bring additional Index Price sources into [Perp Anything](/introduction/trade-on-orderly/permissionless-listing). It is designed for markets that need pricing beyond Orderly's standard CEX feeds and platform oracle sources.
...
Both modes can be selected as Index Price sources during Perp Anything setup. They can also be combined with CEX feeds and platform oracle sources, with each selected source assigned a weight.
...
If a Builder Oracle feed becomes unhealthy, the affected market can be restricted or placed into reduce-only mode according to the Perp Anything risk rules.
...
Both Builder Oracle and BYOK Oracle can be used with supported real-world asset (RWA) markets. RWA market coverage currently includes the **US, Hong Kong (HK), China (CN), and Japan (JP)**. Available market sessions vary by market.

Support for additional RWA markets is planned to be added gradually.
...
For the API sequence, feed setup, source selection, RWA `market_session`, and listing submit example, see [Perp Anything with Custom Oracle](/build-on-omnichain/user-flows/permissionless-listing-custom-oracle).
...
- [Perp Anything with Custom Oracle](/build-on-omnichain/user-flows/permissionless-listing-custom-oracle)
