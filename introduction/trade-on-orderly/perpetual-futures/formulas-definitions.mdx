...
description: "Explore the formulas for margin, collateral, PnL, and liquidation price calculations on Orderly."
---
...
| `IMR_i` | `GET /v1/public/config` | `base_imr` (adjusted by factor) |
| `MMR_i` | `GET /v1/public/config` | `base_mmr` (adjusted by factor) |
...
```ts
/**
 * Consolidated Risk Management Logic
 * Use this to verify account health before and after placing orders.
 */

function calculateAccountHealth(accountInfo, positions, marketConfig) {
  let totalNotional = 0;
  ...
  for (const pos of positions) {
    const notional = Math.abs(pos.qty * pos.markPrice);
    const imr = calculateDynamicRatio(pos.baseImr, pos.imrFactor, notional, accountInfo.maxLeverage);
    const mmr = calculateDynamicRatio(pos.baseMmr, (pos.baseMmr / pos.baseImr) * pos.imrFactor, notional);

    totalNotional += notional;
    totalInitialMargin += notional * imr;
    totalMaintenanceMargin += notional * mmr;
    totalUpnl += pos.unrealizedPnl;
  }

  const totalCollateral = accountInfo.walletBalance + totalUpnl;
  const currentMarginRatio = totalNotional > 0 ? (totalCollateral / totalNotional) : Infinity;

  return {
    isLiquidatable: currentMarginRatio < (totalMaintenanceMargin / totalNotional),
    canOpenNewPosition: totalCollateral > totalInitialMargin,
    withdrawable: Math.max(0, accountInfo.walletBalance - totalInitialMargin - Math.max(0, totalUpnl)),
    liquidationPrice: calculateLiquidationPrice(totalCollateral, totalNotional, totalMaintenanceMargin)
  };
}

function calculateDynamicRatio(baseRatio, factor, notional, maxLeverage = null) {
  const cap = maxLeverage ? 1 / maxLeverage : 0;
  return Math.max(cap, baseRatio, factor * Math.pow(notional, 0.8));
}
```
