...
<Note>Withdrawable balance/collateral does not equal to total or free collateral.</Note>
...
function calculateAccountHealth(accountInfo, positions, marketConfig) {
  ...
  for (const pos of positions) {
    ...
    const imr = calculateDynamicRatio(
      pos.baseImr,
      pos.imrFactor,
      notional,
      accountInfo.maxLeverage
    );
    const mmr = calculateDynamicRatio(
      pos.baseMmr,
      (pos.baseMmr / pos.baseImr) * pos.imrFactor,
      notional
    );
  ...
  return {
    ...
    withdrawable: Math.max(
      0,
      accountInfo.walletBalance - totalInitialMargin - Math.max(0, totalUpnl)
    ),
