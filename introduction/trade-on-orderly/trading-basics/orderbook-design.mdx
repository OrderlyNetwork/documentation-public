...
description: "How Orderly's central limit order book combines CEX-level performance with on-chain settlement."
---
...
Orderly has enabled an omnichain orderbook by splitting the stack into three main components:

**Asset Layer (Asset Vaults)** \- resides on each chain Orderly supports

- Users interact with this layer when registering, depositing, and withdrawing; this is where the funds reside.

**Settlement Layer (Orderly L2)** \- resides on a single chain

- Users do not interact with this directly; it is used as a transaction ledger for storing transaction and user data.

**Engine Layer (Orderbook)** \- Orderbook and order-related services

- Users interact with this when managing orders. This is where trade execution happens; it includes the matching engine, the risk engine, and other services.

Orders from different chains come into the same orderbook, making it chain-agnostic and unifying liquidity across chains, unlike multichain DEXes. After the orders are matched, they are settled on the Orderly L2 chain built using OP Stack, which settles to Ethereum periodically.
