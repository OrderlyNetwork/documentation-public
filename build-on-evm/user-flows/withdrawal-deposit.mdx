...
<Steps>
  <Step title="Approve spending of the token by the Vault smart contract">
    Get Vault & USDC deposit address from [here](/build-on-evm/addresses#arbitrum).
...
<Note>
  ...
  API](/build-on-evm/evm-api/restful-api/private/get-asset-history).
</Note>
...
<Steps>
  <Step title="Choose a valid chain to withdraw your funds to">
    A list of supported chains can be found on our [Smart Contract Addresses page](/build-on-evm/addresses).
  </Step>
  <Step title="Check if the chain has sufficient liquidity">
    Since Orderly Network is an omnichain trading infrastructure, the liquidity of withdrawn assets might need to move across chains. If there is not enough liquidity of the withdrawn asset on the target chain, then an
  ...
  <Step title="Obtain a signature from EIP-712">
    Sign an EIP-712 message of [message type](/build-on-evm/user-flows/wallet-authentication#message-types) `Withdraw`:
