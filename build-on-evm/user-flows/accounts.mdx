...
<Steps>
  <Step title="Choose a valid chain/builder to register the account on">
    List of chains can be found [here](build-on-evm/addresses) and supported builders through [Get list of builders API](/build-on-evm/evm-api/restful-api/public/get-builder-list).
  </Step>
  <Step title="Check if the wallet is already registered or account exists">
     Check if the wallet is already registered via [this](/build-on-evm/evm-api/restful-api/public/check-if-wallet-is-registered) API or if account already exisists via [this](/build-on-evm/evm-api/restful-api/public/check-if-account-exists) API.
  </Step>
