...
<Steps>
  ...
  <Step title="Access key (Orderly key)">
    Your Orderly public key needs to be added to the request header as `orderly-key`. To generate and add a new Orderly key, please refer to [the documentation](/build-on-evm/user-flows/wallet-authentication#orderly-key).
    You can also get Orderly keys from frontend builders like [WOOFi Pro](https://dex.woo.org).
  </Step>
