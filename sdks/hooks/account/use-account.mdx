---
title: "useAccount"
description: "Hook for creating an Orderly account, generating Orderly keys, and managing the KeyStore."
---
...
After the `OrderlyKey` is successfully created, it will be stored in the `KeyStore`, and will be used for API signatures later on. The SDK creates the key with the combined `read,trading` scopes so it can retrieve private account data and perform trading operations. These scopes are independent; `trading` alone does not grant `read`.
...
The `setAddress` method in `Account` will check the `account_id` and the existence of any valid `OrderlyKey` from the provided `address` and update the `state`. If the `state` is `AccountStatusEnum.EnableTrading`, the SDK login process is complete. Access remains limited to the scopes assigned to the key.
