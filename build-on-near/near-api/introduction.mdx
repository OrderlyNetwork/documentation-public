---
title: "Introduction"
description: "Orderly, a permissionless and modular protocol built on NEAR."
---
...
Please set the corresponding header in your request. See refer to [Authentication](/build-on-near/near-api/authentication) for more information.

## Symbol:
Orderly uses the format of `<TYPE>_<BASE>_<QUOTE>` to represent a symbol name, for example: `SPOT_NEAR_USDC.e` means that it is `NEAR_USDC.e` pair in `SPOT` trading market.
...
Specific error codes and messages are defined in [Error Codes](/build-on-near/near-api/error-codes).
