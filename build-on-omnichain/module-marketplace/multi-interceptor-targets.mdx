...
**Prerequisites:** [Tutorial 1](first-plugin) — plugin scaffold and `registerPlugin` basics.
...
**Target:** `"TradingView.Desktop"` (string must match [runtime injector targets](runtime-injector-targets); paths are case-sensitive).
...
| ------------------------- | ------------------------------------- | -------------------------------------------------------------------------------------------------- |
| Widget shows, no menu row | Wrong menu target string              | Compare with [runtime injector targets](runtime-injector-targets) letter by letter |
...
| [Runtime injector targets](runtime-injector-targets) | Confirming exact target path strings while coding |
