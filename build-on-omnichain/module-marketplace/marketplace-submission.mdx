...
**Prerequisite:** Plugin repo with valid `package.json` **name**, GitHub **`origin`** remote (HTTPS form preferred), and metadata aligned with [Manifest and submit](manifest-and-submit). Optional: **`orderly-devkit skills install`** — see below.
...
Full skill–MCP alignment: [Agent skills alignment](agent-skills-alignment); overview: [Skills-first workflow](skills-first).
...
Fix any validation errors (npm name, `repoUrl`, `pluginId`, tags, `usagePrompt` length). See [Manifest and submit](manifest-and-submit).
...
**Verification:** HTTP **201** response — plugin submitted (often status `under_review`). See status table in [Manifest and submit](manifest-and-submit).
...
- Write or refine **`usagePrompt`** for integrators and AI assistants: [Usage prompt and README](usage-prompt-and-readme).
- Short checklist: [Marketplace submit recipe](marketplace-submit-recipe).
