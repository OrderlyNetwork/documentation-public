...
Place `usagePrompt` in `.orderly-manifest.json` (or supply via tooling that writes the manifest). See [Manifest and submit](manifest-and-submit).
...
5. **Configuration**
6. **Interceptors** — table of targets | description (strings must match [Runtime injector targets](runtime-injector-targets))
