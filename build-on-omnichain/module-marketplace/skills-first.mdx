...
2. **Plugin skills** (agent playbooks + triggers):
   ...
   For options (`--skill`, custom source, `npx` forwarding), see [CLI: skills install](cli-reference#skills-install).
...
3. **Orderly SDK Docs MCP** (API truth for `@orderly.network/*` while coding or in chat):
   ...
   See [MCP reference](mcp-sdk-docs). Plugin authors should prefer **`orderly_docs_search`** then **`orderly_docs_get_component`** / **`orderly_docs_get_type`**; older notes may reference a non-existent `orderly_docs_get_hook`—see [Agent skills alignment](agent-skills-alignment).
...
Human tutorials: [T1 First plugin](first-plugin) → [T2 Integrate host](integrate-host) → [T3 Marketplace](marketplace-submission).
...
You can still follow this handbook and the CLI alone: read the same four stages above and use [CLI reference](cli-reference); install **`@orderly.network/plugin-core`** from npm for the full GUIDE/TECH. Installing skills is optional but keeps your future AI sessions aligned with Orderly’s intended flows.

## See also

- [Getting started](getting-started) — installs devkit, skills, MCP, first scaffold.
- [CLI reference: skills](cli-reference#skills-install)
- [Agent skills alignment](agent-skills-alignment) — MCP tool names vs. legacy skill text.
