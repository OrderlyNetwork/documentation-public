...
2. **Plugin skills** (agent playbooks + triggers):
   ...
   For options (`--skill`, custom source, `npx` forwarding), see [CLI: skills install](/build-on-omnichain/module-marketplace/cli-reference#skills-install).
...
3. **Orderly SDK Docs MCP** (API truth for `@orderly.network/*` while coding or in chat):
   ...
   See [MCP reference](/build-on-omnichain/module-marketplace/mcp-sdk-docs). Plugin authors should prefer **`orderly_docs_search`** then **`orderly_docs_get_component`** / **`orderly_docs_get_type`**; older notes may reference a non-existent `orderly_docs_get_hook`—see [Agent skills alignment](/build-on-omnichain/module-marketplace/agent-skills-alignment).
...
Human tutorials: [T1 First plugin](/build-on-omnichain/module-marketplace/first-plugin) → [T2 Integrate host](/build-on-omnichain/module-marketplace/integrate-host) → [T3 Marketplace](/build-on-omnichain/module-marketplace/marketplace-submission).
...
You can still follow this handbook and the CLI alone: read the same four stages above and use [CLI reference](/build-on-omnichain/module-marketplace/cli-reference); install **`@orderly.network/plugin-core`** from npm for the full GUIDE/TECH. Installing skills is optional but keeps your future AI sessions aligned with Orderly’s intended flows.

## See also

- [Getting started](/build-on-omnichain/module-marketplace/getting-started) — installs devkit, skills, MCP, first scaffold.
- [CLI reference: skills](/build-on-omnichain/module-marketplace/cli-reference#skills-install)
- [Agent skills alignment](/build-on-omnichain/module-marketplace/agent-skills-alignment) — MCP tool names vs. legacy skill text.
