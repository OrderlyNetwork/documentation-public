...
description: "Implement secure request signing for the Orderly API using the ed25519 standard."
---
...
class Signer(object):
    ...
    def sign_request(self, req: Request) -> PreparedRequest:
        ...
        message = str(timestamp) + req.method + path_with_query + json_str
...
const orderlyKey = base58.decode(process.env.ORDERLY_SECRET!);
...
export function signAndSendRequest(
  ...
  return fetch(input, {
    headers: {
      ...
      'orderly-timestamp': String(timestamp),
      'orderly-account-id': orderlyAccountId,
      'orderly-key': `ed25519:${encodeBase58(ed25519.getPublicKey(privateKey))}`,
...
Orderly validates every authenticated request through four checks. A request must pass all four to be accepted.
