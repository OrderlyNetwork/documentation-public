...
Pre-TGE Listing extends [Perp Anything](/build-on-omnichain/perp-anything/introduction) to tokens that have not launched yet -- no spot market, no external venue, and no price feed to aggregate. The standard flow requires at least one exchange (CEX/DEX) or oracle source, which a pre-launch token cannot provide. Pre-TGE Listing opens a separate entry path with a synthetic price source and tightened risk caps, so a Builder can open price discovery before the token's formal TGE.
...
Pre-TGE Listing reuses the Perp Anything pipeline, with three additions: a dedicated entry path, a synthetic price source, and tightened risk caps.

### Creating the listing

In the [Perp Anything listing wizard in Orderly One](https://dex.orderly.network/permissionless-listing), the Builder selects **Pre-TGE Token** on the "What are you listing?" step, alongside Crypto and RWA. Before this, the Builder aligns with Orderly offline on the token ticker and a reference initial price (from FDV, OTC quotes, or a comparable launch). Orderly then sets up the Pre-TGE Oracle for that token, which the Builder selects as the market's price source.
...
1. Add external sources (CEX / DEX / oracle / BYOK) and adjust weights. See [Builder Oracle](/build-on-omnichain/user-flows/builder-oracle) for Builder-controlled sources.
...
<Note>
  The `is_pretge` field is not currently included in this documentation site's bundled OpenAPI
  reference. Confirm its production response location with your Orderly integration contact before
  implementation.
</Note>
