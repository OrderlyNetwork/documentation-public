...
Pre-TGE Listing extends [Perp Anything](/build-on-omnichain/perp-anything/introduction) to tokens that have not launched yet -- no spot market, no external venue, and no price feed to aggregate. The standard flow requires at least one CEX or oracle source, which a pre-launch token cannot provide. Pre-TGE Listing opens a separate entry path with a synthetic price source and tightened risk caps, so a Builder can open price discovery before the token's formal TGE.
...
1. Add external sources (CEX / oracle / BYOK) and adjust weights. See [Builder Oracle](/build-on-omnichain/user-flows/builder-oracle) for Builder-controlled sources.
