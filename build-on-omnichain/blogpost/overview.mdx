...
These endpoints are hosted by the DEX Creator API. Append the complete path, for example `https://dex-api.orderly.network/api/public/v1/blogposts`. Content, IDs, slugs, and cursors are environment-specific.
...
| Builder platform   | Supply `brokerId` | Published posts for that broker, including posts available only in its platform feed |

A builder's latest published version may be newer than the version approved for Orderly's official feed. The same slug can therefore return different content and timestamps depending on `brokerId`. Use the same `brokerId` for list and detail requests. A broker-scoped request filters to that broker and does not automatically add posts from Orderly's official feed.

Builder-only visibility controls feed distribution. The endpoints remain public, so a broker ID is not a credential or confidentiality control. Draft, deleted, and unpublished posts are not returned.

## Sorting and pagination

The list sorts by `startsAt` when present, otherwise `publishedAt`, descending, then by post ID descending. A start time takes precedence even when earlier than publication time. Future start times are included. Sorting uses the revision visible in the requested scope; draft changes and revisions awaiting approval for Orderly's official feed do not change that feed's ordering.
...
Public Blogpost endpoints share **60 requests per client IP per rolling 60 seconds at each API instance**. Coordinate requests and cache results, especially when users share an outbound IP.
...
| `409`         | Incomplete approved revision on detail. Show temporary unavailability and retry later.                             |
