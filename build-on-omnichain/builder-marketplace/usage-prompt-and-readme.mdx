...
## `usagePrompt` (Builders Marketplace / manifest)
...
Place `usagePrompt` in `.orderly-manifest.json` (or supply via tooling that writes the manifest). See [Manifest and submit](/build-on-omnichain/builder-marketplace/manifest-and-submit).

## GitHub README

The README is **not** uploaded to the Builders Marketplace API, but it is the human-facing home for your repo. Suggested structure:
...
5. **Configuration**
6. **Interceptors** — table of targets | description (strings must match [Runtime injector targets](/build-on-omnichain/builder-marketplace/runtime-injector-targets))
