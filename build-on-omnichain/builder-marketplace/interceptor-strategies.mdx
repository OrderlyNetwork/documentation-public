...
Define a child component that _is_ a React component and use Hooks there:
...
There is **no** `orderly_docs_get_hook` tool in the current server; use search + get_component/get_type instead ([alignment notes](/build-on-omnichain/builder-marketplace/agent-skills-alignment)).
