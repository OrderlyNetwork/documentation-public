...
2. Check if the wallet is already registered via [this](/build-on-omnichain/restful-api/public/check-if-wallet-is-registered) API or if account already exists via [this](/build-on-omnichain/restful-api/public/check-if-account-exists) API.
...
1. Send all the necessary information via [Add Orderly key](/build-on-omnichain/restful-api/public/add-orderly-key) API
