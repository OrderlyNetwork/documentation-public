...
Access Key scopes are independent. Granting one scope does not grant any other scope:
...
<Warning>
  For a normal trading integration that also reads balances, positions, or other private account
  data, assign both scopes as `read,trading`. Add `asset` separately when the integration needs
  asset operations.
</Warning>
...
| scope      | string    | Y        | Valid scopes are `read`, `trading`, and `asset`. Scopes are independent. Send multiple scopes as a comma-separated string, such as `read,trading`. |
| timestamp  | timestamp | Y        | Timestamp in UNIX milliseconds                                                                                                                     |
