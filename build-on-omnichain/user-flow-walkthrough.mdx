...
> [!NOTE]
> **API Privilege Model & Key Scope Design (By Design)**:
>
> - **`read` Scope**: Allows reading private user metrics, plus invoking account configuration changes (such as adjusting leverage via `POST /v1/client/leverage` or editing maintenance configs via `POST /v1/client/maintenance_config`). Decoupling these configuration actions from order execution is a deliberate architectural decision (**By Design**).
> - **`trading` Scope**: Mandatory strictly for order placement and lifecycle execution.
