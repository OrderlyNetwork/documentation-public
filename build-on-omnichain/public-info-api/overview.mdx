...
The examples below hit Mainnet. To run them against Testnet, swap the host for `testnet-api.orderly.org`.
...
<Note>
All address-level data exposed here — positions, open orders, fills, deposits, withdrawals, PnL — is **public by design**. Any caller can query any wallet without permission. Builders should not treat these fields as private when building on top of this API.
</Note>
...
| Environment | URL |
| --- | --- |
| Mainnet | `POST https://api.orderly.org/v1/public/query` |
| Testnet | `POST https://testnet-api.orderly.org/v1/public/query` |
...
Every request goes to the same path; the `type` field in the JSON body selects the handler.
