...
| `total_open_interest` | string \| null | Sum across markets; `null` only if every symbol misses the position-monitor cache |
...
- **Cold start (~1s after pod restart):** falls back to a Redis scan + `symbols` table; first call after deploy may be slower
