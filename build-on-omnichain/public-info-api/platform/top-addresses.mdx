...
| `pnl_7d`, `pnl_30d`, `volume_7d`, `volume_30d` | string | Calendar windows: today (MySQL) plus the prior `N − 1` days (Trino), UTC-day-aligned |
| `win_rate_7d`, `win_rate_30d` | double \| null | Sourced from Trino; `null` if Trino is unavailable |
...
- If Trino is unavailable, the snapshot is still served — 7d / 30d win-rate fields fall back to `null`
