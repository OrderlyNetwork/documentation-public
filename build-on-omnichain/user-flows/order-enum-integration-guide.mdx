...
If `pair_overrides` includes the order symbol, use that symbol-specific order tag fee rate. Otherwise, use `default_fee_rate`. Display the order tag fee separately from the standard trading fee so users understand both components before placing an order.
...
| Order tag fee config | `enum:STRATEGY_DCA` | Adds the configured order tag fee on top of existing fees and preserves referral. |
...
| Field              | Description                                                                                      |
| ------------------ | ------------------------------------------------------------------------------------------------ |
| `enum_id`          | Unique uppercase order tag ID, for example `STRATEGY_DCA`. The API field name remains `enum_id`. |
...
- Treat `enum_id` as immutable. If the classification semantics change, create a new order tag rather than reusing a misleading ID.
