...
When `order_tag` uses the `enum:<UPPERCASE_ID>` format, the matching order tag fee config applies an additional custom trading fee on top of the standard fee schedule. This is different from a plain referral-code `order_tag`, which is treated as referral attribution and does not apply the custom trading fee per order.
