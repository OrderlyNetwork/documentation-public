...
<Warning>
  - There can only be **one EOA** per Smart Contract account. If a new Delegate Signer is announced, the old one will be deleted.
  - Smart Contract accounts are **single-chain** — they can only deposit/withdraw from the chain where the Delegate Signer call was made.
