...
Builder Oracle lets Builders bring their own Index Price sources into [Perp Anything](/build-on-omnichain/perp-anything/introduction). Use it when a market needs pricing beyond Orderly's standard exchange feeds (CEX/DEX) and platform oracle sources.
...
Both modes can be combined with exchange feeds (CEX/DEX) and platform oracle sources. Each selected source receives a weight in the market's Index Price configuration.
...
| :--- | :--- | :--- |
| **CEX** | Centralized exchange price feeds supported by Orderly. | Binance, Binance Futures, OKX, Bybit, Gate.io, KuCoin, Coinbase, MEXC, Bitget, BingX, LBank |
| **DEX** | Decentralized exchange price feeds supported by Orderly. | Hyperliquid |
...
Orderly supports price feeds from centralized (CEX) and decentralized (DEX) exchanges:

| Exchange | Type | Source Identifier |
| :--- | :--- | :--- |
| Binance | CEX | `BINANCE` |
| Binance Futures | CEX | `BINANCE_FUTURES` |
| OKX | CEX | `OKX` |
| Bybit | CEX | `BYBIT` |
| Gate.io | CEX | `GATEIO` |
| KuCoin | CEX | `KUCOIN` |
...
```
  ...
  <Step title="Fetch available price sources">
    Call `GET /v1/broker/listing/index_sources` to retrieve the available source list. The response can include CEX, DEX, platform oracle, Builder Oracle, and BYOK Oracle sources.
