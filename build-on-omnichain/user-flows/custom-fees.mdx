...
**Base Fee:** Orderly retains 100% of the 3 bps taker fee, and there's no maker fee. The base fee is subject to change according to our [builder staking programme](/introduction/trade-on-orderly/trading-basics/trading-fees#builder-staking-programme).
...
<Steps>
  ...
  <Step title="Setting the fees">
    ...
    2. **For users re-assigned to "default" fee rate:** send the list through [POST `/v1/broker/fee_rate/set_default`](/build-on-omnichain/restful-api/private/reset-user-fee-rate)

    3. **For users assigned to new fee rate:** send the list through [POST `/v1/broker/fee_rate/set`](/build-on-omnichain/restful-api/private/update-user-fee-rate). Send updates in batches, grouped by the same `maker_fee` `taker_fee` combination.
    <Note>
    ...
    Retrieve user's fee rate through [GET `/v1/broker/user_info`](/build-on-omnichain/restful-api/private/get-user-fee-rates).

    Retrieve default fee rate through [GET `/v1/broker/fee_rate/default`](/build-on-omnichain/restful-api/private/get-default-builder-fee).
...
<Accordion title="How are fee rebates distributed?">
  ...
  provided during the builder ID setup.
</Accordion>
