...
<Note>
  The base fee is subject to change according to the [builder staking programme](/introduction/trade-on-orderly/trading-basics/trading-fees#builder-staking-programme). Higher tiers can receive negative maker fees (maker rebates).
</Note>
...
<Steps>
  ...
  <Step title="Calculate fee rates">
    ...
    <Note>
      The fee rate is the total charged to the user (Orderly base fee + custom fee). For taker fees, the custom fee cannot be less than the Orderly base fee. For maker fees, builders can set negative maker fees (rebates) for users when the builder's base maker fee is negative.
    </Note>
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title="How does the rebate work for builders?">
    Builders keep all fees above the Orderly base fee. With negative maker fees (rebates), the builder receives the maker rebate from Orderly and can choose to pass it on to users.
  </Accordion>
