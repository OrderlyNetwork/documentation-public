---
title: "Perp Anything with Custom Oracle"
description: "How Builders can list a market using custom oracle feeds, Bring Your Own Key oracle sources, and RWA market sessions."
---

This flow is for Builders who want to list a Perp Anything market and use a custom oracle feed as one of the market's Index Price sources.

Use this page together with the [Perp Anything overview](/introduction/trade-on-orderly/permissionless-listing). The overview explains Builder responsibility, Insurance Fund requirements, Isolated Margin behavior, and trader-facing risk disclosures. This page focuses on the integration flow and API sequence.

<Note>
  RWA market coverage currently includes the US, Hong Kong (HK), China (CN), and Japan (JP).
  Available market sessions vary by market. Support for additional markets is planned to be added
  gradually.
</Note>
...
```
    ...
    For an RWA listing, use a `market_session` value returned by this endpoint. Each response row includes the session name, timezone, trading hours, and holiday and early-close information.
