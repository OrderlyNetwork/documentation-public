...
<Steps>
  ...
  <Step title="Check if the wallet is already registered or account exists">
     Check if the wallet is already registered via [this](/build-on-omnichain/evm-api/restful-api/public/check-if-wallet-is-registered) API or if account already exists via [this](/build-on-omnichain/evm-api/restful-api/public/check-if-account-exists) API.
  </Step>
